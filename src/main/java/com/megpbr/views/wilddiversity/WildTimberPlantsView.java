...
@PageTitle("Timber Plants")
@Route(value = "format-26", layout = MainLayout.class)
@Uses(Icon.class)
public class WildTimberPlantsView extends HorizontalLayout {
	...
	public WildTimberPlantsView(Dbservice service, CropService cservice, UserService uservice) {
		this.dbservice = service;
		this.cservice = cservice;
		this.uservice = uservice;
		// this.auditservice=auditservice;
		format = dbservice.getFormat(26);
		setSizeFull();
		ConfigureGrid();
		ConfigureForm();
		ConfigureCombo();
	...
	public Component ConfigureGrid() {
		try {
			...
			grid.addColumn("uses").setHeader("Other Uses if Any").setAutoWidth(true).setResizable(true).setSortable(true);
