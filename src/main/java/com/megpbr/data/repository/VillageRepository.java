...
public interface VillageRepository extends JpaRepository<Village, Long> {
	...
	@Query("SELECT vv AS village, bb AS block, dd AS district, ss AS state, " +
		       "COALESCE(x.cnt, 0) + COALESCE(s.cnts, 0) + COALESCE(m.cntm, 0) AS sum " +
		       "FROM Village vv " +
		       "JOIN vv.block bb " +
		       "JOIN bb.district dd " +
		       "JOIN dd.state ss " +
		       "LEFT OUTER JOIN (SELECT x.village.villageCode AS villageCode, COUNT(*) AS cnt " +
		       "FROM Crops x GROUP BY x.village.villageCode) x " +
		       "ON vv.villageCode = x.villageCode " +
		       "LEFT OUTER JOIN (SELECT s.village.villageCode AS villageCode, COUNT(*) AS cnts " +
		       "FROM Scapes s GROUP BY s.village.villageCode) s " +
		       "ON vv.villageCode = s.villageCode " +
		       "LEFT OUTER JOIN (SELECT m.village.villageCode AS villageCode, COUNT(*) AS cntm " +
		       "FROM Markets m GROUP BY m.village.villageCode) m " +
		       "ON vv.villageCode = m.villageCode " +
		       "ORDER BY vv.villageName")
		List<Object[]> villageCount();

	@Query("SELECT b as block, dd AS district, ss AS state, COALESCE(vx.cropsSum, 0) + COALESCE(vy.scapesSum, 0) + COALESCE(vz.marketsSum, 0) AS Sum " +
		       "FROM Block b " +
		       "JOIN b.district dd " +
		       "JOIN dd.state ss " +
