...
@PageTitle("Wild Relatives Crops")
@Route(value = "wildrelativescrops", layout = MainLayout.class)
@Uses(Icon.class)
public class WildRelativesView extends VerticalLayout{
	...
	public WildRelativesView(Dbservice service, CropService cservice) {
		this.dbservice=service;
		this.cservice=cservice;
		format=dbservice.getFormat(23);
		setSizeFull();
		ConfigureGrid();
		ConfigureForm();
		ConfigureCombo();
	...
	public Component ConfigureGrid() {
		try {
			...
			grid.addColumn("type").setAutoWidth(true).setHeader("Associated Crop").setAutoWidth(true).setResizable(true).setSortable(true);
			...
			grid.addColumn("otherDetails").setHeader("Other Details(Modeof Usage)").setAutoWidth(true).setResizable(true).setSortable(true);
